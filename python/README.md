...
<br>
1.&emsp;&emsp;7 Python解释器<br>
&emsp;&emsp;不同平台，不同cpu上，有不同的解释器，简单说来，就是让python最终控制0和1,不要纠结过程。如果控制失败，解释器会报错。<br>
<br>
2、&emsp;&emsp;数据类型<br>
2.&emsp;&emsp;1 数值类型 (Numeric)：包括整数 int（如 10）、浮点数 float（如 3.14）和复数 complex。<br>
...
<br>
2.&emsp;&emsp;2 字符串类型 (String)：使用单引号或双引号括起来的文本 str（如 "Hello"）。<br>
...
&emsp;&emsp;示例<br>
1.&emsp;&emsp;整数 %d (常用语数量、ID)<br>

```python
print("我有 %d 个苹果。" % 5)
```

2.&emsp;&emsp;浮点数 %f (常用语金额、百分比)<br>
...
3.&emsp;&emsp;字符串 %s (最通用，万物皆可 %s)<br>

```python
print("欢迎来到 %s 的频道！" % "chanvel")
```

4.&emsp;&emsp;十六进制 %x (常用于颜色代码、内存地址)<br>
...
<br>
2.&emsp;&emsp;3 布尔类型 (Boolean)：只有两个值：True 或 False，常用于逻辑判断。<br>
...
<br>
2.&emsp;&emsp;4 序列类型 (Sequence)：包括可变的列表 list（如 [1, 2]）和不可变的元组 tuple（如 (1, 2)）。<br>
2.&emsp;&emsp;4.1 list列表，有序队列，可以索引，也就是找到对应人，一般排队到队尾，也可插队，还有删除、弹出、排序等操作<br>
&emsp;&emsp;示例A<br>
...
<br>
2.&emsp;&emsp;4.2 tuple元组，list是可以随意修改的阵列，而元组是一旦创建就不能修改的只读阵列。<br>
...
<br>
2.&emsp;&emsp;5 映射与集合 (Mapping & Set)：包括存储键值对的字典 dict（如 {"key": "value"}）和无序不重复的集合 set。dict是找东西，set是比关系。<br>
2.&emsp;&emsp;5.1 在python中的dict,在其他语言中是map,核心逻辑是KV，也就是key-value键值存储。<br>
...
<br>
2.&emsp;&emsp;5.2 set集合，就是只有key没有value的dict。底层实现上都是hash table哈希表实现功能。<br>
&emsp;&emsp;示例<br>
&emsp;&emsp;绝活：去重<br>
...
<br>
2.&emsp;&emsp;6[]list,()tuple,{}dict/set<br>
&emsp;&emsp;示例<br>
1.&emsp;&emsp;[] 列表 (List) - 最常用，可随便改<br>
...
2.&emsp;&emsp;() 元组 (Tuple) - 定义了就不能动<br>
...
3.&emsp;&emsp;{} 字典 (Dict) - 键值对，查得快<br>
...
4.&emsp;&emsp;{} 集合 (Set) - 只要不重复的元素<br>
...
变量、&emsp;&emsp;常量作为动态语言，可以是任何数据类型，单个数据、组合数据都可以。<br>
...
<br>
5、&emsp;&emsp;条件判断<br>
5.&emsp;&emsp;1 if then/用:进行了替换，并且换行强制向右缩进<br>
&emsp;&emsp;示例<br>
...
6.&emsp;&emsp;1 确定性循环是指循环次数在开始时就已确定的情况，通常使用 for...in 循环来实现。<br>
&emsp;&emsp;示例<br>
...
<br>
6.&emsp;&emsp;2 非确定性循环是指循环次数在开始时不确定，通常使用 while 循环来实现，直到满足某个条件才停止。<br>
&emsp;&emsp;示例<br>
...
### 二、函数
1、&emsp;&emsp;万物皆函数<br>
1.&emsp;&emsp;1 “Everything is a function”，约翰·麦卡锡 (John McCarthy) 的追随者，把这个作为信仰，他们认为“状态是万恶之源，变换才是宇宙真理”。与之对应的是面向对象编程的 “Everything is an object”。<br>
1.&emsp;&emsp;2 其实这就是运动的还是静止的哲学问题，静止的是物质，运动的是能量。而我们知道，Albert Einstein爱因斯坦著名质能方程E = mc^2，告诉我们物质和能量是可以相互转换的。从这个角度上来说，everything is a function 还是 object,也就不难理解了。生命的意义，在于确定某个目标的存在（object），实现这个目标（function）。人的空虚感常常源于静止地看待世界，而忽略了运动的本质，生命不息，运动不止。<br>
1.&emsp;&emsp;3 函数式编程强调的正是“能量”的流动，而不是“物质”的堆积。如果想要改变output,只能改变input,或者改变函数f()本身。衍生出来的解耦思维，可以把大问题拆解成小问题，把复杂拆解成简单。如果某个环节出现问题，可以单独替换这个环节，而不影响整体。<br>
<br>
2、&emsp;&emsp;数学维度<br>
&emsp;&emsp;y = f(x)<br>
2.&emsp;&emsp;1 自变量x，函数f，因变量y<br>
2.&emsp;&emsp;2 一元函数，二元函数，多元函数<br>
2.&emsp;&emsp;3 映射关系：单射，满射，双射<br>
2.&emsp;&emsp;4 函数的运算：加法，减法，乘法，除法，复合运算<br>
2.&emsp;&emsp;5 反函数<br>
2.&emsp;&emsp;6 函数的性质：奇偶性，周期性，单调性，上下界，连续性<br>
2.&emsp;&emsp;7 函数的图像<br>
2.&emsp;&emsp;8 初等函数与特殊函数<br>
2.&emsp;&emsp;9 函数的极限与导数<br>
2.&emsp;&emsp;10 积分与微分方程<br>
2.&emsp;&emsp;11 多元函数与偏导数<br>
2.&emsp;&emsp;12 向量值函数与曲线积分<br>
2.&emsp;&emsp;13 函数空间与泛函分析<br>
2.&emsp;&emsp;14 常见函数类型：线性函数，二次函数，指数函数，对数函数，三角函数，反三角函数，超越函数等<br>
2.&emsp;&emsp;15 应用领域：物理学，工程学，经济学，生物学，计算机科学等<br>
&emsp;&emsp;看着是不是头大了，编程语言中的函数，其实就是数学中函数的简化版，主要是为了实现代码的复用和模块化设计。编程只是工具，编程水平的高低，决定于对问题的理解和抽象能力，而不只是对语法的熟练程度。<br>
<br>
3、&emsp;&emsp;调用函数<br>
3.&emsp;&emsp;1 调用函数，看着很高级，实际很好理解。你用锅炒菜，不会从挖矿石冶铁锻造锅开始吧？直接调用锅来炒菜，那么这个锅就是函数，调用锅就是调用函数。参数就是你放进锅里的菜和调料，返回值就是炒好的菜。<br>
3.&emsp;&emsp;2 常用的内置函数：print(), len(), type(), int(), str(), list(), dict(), set(), range(), input(), sum(), max(), min(), sorted(), map(), filter(), reduce()等。<br>
<br>
4、&emsp;&emsp;定义函数<br>
4.&emsp;&emsp;1 继续用炒菜的例子，定义函数就是你自己造锅，你要先有个设计图纸（函数名和参数列表），然后去买材料（函数体），最后把锅造好（return返回值）。以后你要炒菜，就可以直接用你自己造的锅了。<br>
4.&emsp;&emsp;2 定义函数的语法：<br>
...
<br>
4.&emsp;&emsp;3 参数类型：位置参数，默认参数，可变参数，命名关键字参数，关键字参数。python解释器会按照这个顺序依次匹配参数。可以微调顺序。<br>
4.&emsp;&emsp;3.1  位置参数：按顺序传递参数<br>
...
4.&emsp;&emsp;3.2 默认参数：给参数设置默认值，定义的默认函数是不变的，但是调用的时候可以灵活多变。<br>
...
4.&emsp;&emsp;3.3 可变参数：传递任意数量的位置参数<br>
...
4.&emsp;&emsp;3.4 命名关键字参数：指定关键字参数的名称<br>
...
4.&emsp;&emsp;3.5 关键字参数：传递任意数量的关键字参数<br>
...
1.&emsp;&emsp;位置参数：主食<br>
2.&emsp;&emsp;默认参数：小菜<br>
3.&emsp;&emsp;可变参数：加料（打包成元组）<br>
4.&emsp;&emsp;命名关键字：龙虾（必须指定名字）<br>
5.&emsp;&emsp;关键字参数：备注（打包成字典）<br>
...
<br>
4.&emsp;&emsp;4 返回值类型：单个值，多个值，None<br>
4.&emsp;&emsp;5 充分利用pass,如果def时候还没想好函数体，可以用pass占位，表示什么都不做，等以后再补充。<br>
...
4.&emsp;&emsp;6 在交互环境中def函数时候，会有... 提示符，表示函数体还没有结束。回车两次，函数定义结束<br>
<br>
5、&emsp;&emsp;递归函数：函数自己调用自己，先递再归<br>
&emsp;&emsp;示例<br>
...
1.&emsp;&emsp;让路，把上面的 n-1 个盘子从 A 搬到 B，借助 C<br>
2.&emsp;&emsp;到位，把最下面那个最大的盘子从 A 搬到 C<br>
3.&emsp;&emsp;重聚，把留在 B 的 n-1 个盘子搬到 C，借助 A<br>
...
1.&emsp;&emsp;[n=3] 把上面2个从 A->B:<br>
&emsp;&emsp;- A --> C<br>
&emsp;&emsp;- A --> B<br>
&emsp;&emsp;- C --> B<br>
2.&emsp;&emsp;[n=3] 把最下面最大的从 A->C:<br>
&emsp;&emsp;- A --> C<br>
3.&emsp;&emsp;[n=3] 把留在 B 的2个从 B->C:<br>
&emsp;&emsp;- B --> A<br>
&emsp;&emsp;- B --> C<br>
&emsp;&emsp;- A --> C<br>
&emsp;&emsp;最终输出：<br>
...
---
made by chanvel   |   2025-12-28 12:42